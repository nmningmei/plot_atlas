...
def create_mesh(mask_list:List,stat_mesh:str,radius:int = 0,kind:str = 'line',) -> np.ndarray:
    """
...
precuneus""".split('\n')
    ...
    for ii,(color,mask_name) in enumerate(zip(color_list,roi_names)):
        ...
        temp[temp > 0] = ii + 1
        mask_items.append(image.new_img_like(mask[0],np.array(temp > 0,dtype = np.int32)))
